...
install.packages("BiocManager") #need to install BiocManager (on CRAN) before you can install Bioconductor
...
```{r}
BiocManager::valid() # at the bottom it tells you which packages are out of date and the code you need to run to update and install them. Send this to genmail?
```
...
```{r}
library(Biostrings) # defines the classes of a character string which can only consist of A,T,G,C
```
...
```{r}
DNA_ALPHABET #different letters stand for purines/pyrimadines?
```
...
```{r}
IlluminaQuality(0:40) #lower case letters = higher quality
```
...
readDNAStringSet("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa", format="fasta") #use read DNA String Set to read in a fasta file
...
```{r}
Ebox <- "CANNTG"
countPattern(Ebox, chrY, fixed=FALSE) #fixed = FALSE means that the N doesn't mean N, it means this base can be anything. Whereas if you put true it expects there to be an N in the sequence.
```
...
#didn't specify a strand so it gives you "*" meaning it can be either
```

**What genomics file format does this remind you of?**

> Answer: Similar to a BED file format
> 
...
```{r}
seqnames(demo_granges) # give me everything no matter how many times it appears
```
...
```{r}
seqlevels(demo_granges) # give me the individual ones
```
...
```{r}
# Note:
# Bioconductor has a different DataFrame function compared to base R (data.frame). This DataFrame doesn't work with ggplot, you've have to convert it to a data.frame using as.data.frame(...).

mcols(demo_granges) #have to look for the column names first before you can pull out one of the columns (below)

```

```{r}
demo_granges$metadata1 # pulling out metadata1 column from demo_granges
```

```{r}
# tells us what type of data is in the columns (e.g. list). A dataframe has metadata on its dataframe columns.Converts columns to rows so that the columns become metadata. This is unique to Bioconductor. 
# if you do as.data.frame you lose the metadata.
...
```{r}
mcols(gtf_data)
colnames(mcols(gtf_data)) # in this case we want to column names of the metadata column names (mcols takes the metadata of the dataframe) so we use colnames and the mcols functions
```
...
unique_transcripts <- unique(gtf_ACTB$transcript_id) # use unique to get the number of unique transcripts
unique_transcripts
unique_transcripts <- na.omit(unique_transcripts) # rerun it with the na.omit to remove the NA value that comes up orignially
length(unique_transcripts) # use length function to get the number of unique transcripts
...
```{r}
gtf_ACTB_exons <- subset(gtf_ACTB, type=='exon') # subset the gtf_ACTB based on exon
length(gtf_ACTB_exons)

gtf_ACTB_transcripts <- split(gtf_ACTB_exons, gtf_ACTB_exons$transcript_id) # split the gtf_ACTB_exons based on the transcript ID
gtf_ACTB_transcripts # the class of the returned object is a list
```
...
```{r}
class(org.Hs.eg.db) # object is of the class Orgdb, which means you can use the functions columns, keytypes etc on this.
```
...
```{r}
keytypes(org.Hs.eg.db) # every column in the table can be used to get information
```
...
# head works in this case as there are 6 keys per row so you wouldn't need the n argument, but can change this value n to however many you need. Could also use x[1:6], which is more flexible as you can use x[5:10] or x[c(2,4,105)] to get the 2nd, 4th and 105th keys (where x = keys(org.Hs.eg.db, 'SYMBOL'). 
  
human_symbols <- head(keys(org.Hs.eg.db, 'SYMBOL'), n=6) 
...
```{r}
# from select() help page: select(hgu95av2.db, keys=keys, columns = c("SYMBOL","GENETYPE"))
# select(org.db, keys, columns, keytype)

AnnotationDbi::select(org.Hs.eg.db, human_symbols, keytype= "SYMBOL", c("ENTREZID", "ENSEMBL"))
```

  + What happens if you do not specify `AnnotationDbi`?

> Answer: Nothing, it looks the same? If packages are not loaded in the right order this will fail. Here, this is because we loaded the Bioconductor package was loaded first. 
> 

- Use the `mapIds()` method to get the Ensembl identifier for those gene symbols.

```{r}
# mapIds(x, keys, columns, keytype)
# adding the multiVals argument = list generates a list instead of a vector

mapIds(org.Hs.eg.db, human_symbols, "ENSEMBL", "SYMBOL", multiVals = "list")
```
...
```{r}
listMarts() #lists what is available today on the BioMart website
```

- Use the `listEnsemblArchives()` function to lists the available archived versions of Ensembl.

```{r}
listEnsemblArchives() # want to be able to access old versions. This provides a URL which is the important bit. Star indicates which is the current release.
```
...
# in the help section there are lots of optional arguments, if you don't know what you want to do just leave them out and they'll be set to default 
# we didn't select a specific dataset but there are lots of datasets in it - see next section
...
```{r}
list_datasets <- listDatasets(mart)
list_datasets
```
...
# now that we know what dataset me need - used command + click on the list_datasets object. This then opens in a seperate window and we filtered it under description for human

mart <- useMart(biomart="ENSEMBL_MART_ENSEMBL", host="https://may2021.archive.ensembl.org", dataset= "hsapiens_gene_ensembl")
...
```{r}
# we have 3,164 attributes with 3 columns of name, prescription and page
list_new_attributes <- listAttributes(mart)
View(list_new_attributes)
...
```{r}
# again just open the list_new attributes with View (shown in above cells)
listFilters(mart) # use this to find the filter with within the mart
...
# here create a matrix containing the numbers 1-8,4 rows and 2 columns; then creating dummy raw data and giving a symbol and length to each gene, called them controls or treated; then given attributes of ages 18 or 20
library(SummarizedExperiment)
...
# then this section is what you'll usually need to do, which is assigning the counts to the count matrix, the rowData to rowData generated above, and colData to coldata generated above
...
```{r, include=TRUE}
demo_se # when we run it we see the effects of the last section of code applied to the dataset
```

Use the methods `assays()`, `assayNames()` `assay()`, `colData()`, `rowData()`, `mcols()` and `metadata()` on the object `demo_se`.

```{r}
# these are methods that the user will usually like to access
# summarised experiment combines 3 objects (row data, col data and expt data) into one object 
# counts as matrix, the other two as data frames (rowData and colData)

assays(demo_se) # one list generated
assayNames(demo_se)
assay(demo_se, "counts") # when you want to extract one assay, this gives you one matrix
colData(demo_se) # now I want to extract the colData and do something with it. This is a dataframe.
rowData(demo_se) # same as above
mcols(demo_se) # same as rowData - don't use this use rowData
metadata(demo_se) # experiment data (expt data shown on slide) empty list because we haven't defined any metadata
...
rownames(assay_counts) <- assay_counts$X # here we assign the rownames as the column X in assay_counts
assay_counts$X <- NULL # then to avoid repeating columns, we can now delete the X column (or fill it with nothing NULL)
assay_counts <- as.matrix(assay_counts) # finally we need to turn it into a matrix
...
```{r}
# read_csv will give you a tibble - want to avoid this so we use base r which is read.csv
# row.names =1 takes the first column it finds and uses that as the row names

sample_metadata <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/sample_metadata.csv", row.names = 1) 
...
```{r}
# same as above but just importing the gene metadata

gene_metadata <- gene_metadata <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/gene_metadata.csv", row.names = 1) 
...
```{r}
# gene metadata is the rows as in the original data matrix its in the rows and the sample metadata is in the columns

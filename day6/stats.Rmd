...
> Answer: If we increase the n number it will be closer to the mean value we are looking for
...
```{r}
summary(y) #broken up value into 4: minimum, 1st quartile, median, 3rd quartile and maximum.  
```
...
```{r}
# seq generates a vector, starting at 0, ending in 1 and increasing in size of 0.5
quant <- quantile(y, seq(0,1,0.1))
quant
```
...
```{r}
hist(y, breaks=20) #breaks is used to determine the number of bins that you want 
```
...
abline(v=median(y), col="red", lty=2)
abline(v=mean(y)-sd(y), col="blue")
abline(v=mean(y)+sd(y), col="blue")
abline(v=median(y)-mad(y), col="blue", lty=2)
abline(v=median(y)+mad(y), col="blue", lty=2)

#use abline to generate a straight lines in a plot, use v= to make a vertical line. Use col="red" in quotes to colour the line red. Needs to be in quotes otherwise it'll look for a variable. To add more than one line you need to do abline again for each new line. Then you have to run the whole script together rather than one line at a time. 

#lyt=2 is used when you want a dotted line
...
```{r}
a<- seq(-5,5,0.1) # see the word range and think you need to make a sequence from -5 to 5 by increments of 0.1
norm_data <- data.frame(x=a, cdf=pnorm(a)) # cumulative distribution frequency = the probability of normal disrtibution in sequence "a"
ggplot(norm_data) + 
  geom_line(aes(x=a, y=cdf)) +
geom_point(aes(x=a, y=cdf)) # plot as a line and as a point
...
a<- seq(0,1,0.01) 
norm_data <- data.frame(x=a, cdf=qnorm(a)) # use qnorm for inverse cumulative distribution
ggplot(norm_data) + 
  geom_line(aes(x=a, y=cdf)) +
...
a<- seq(-5,5,0.1) 
norm_data <- data.frame(x=a, cdf=dnorm(a)) # use dnorm for the density function
ggplot(norm_data) + 
  geom_line(aes(x=a, y=cdf)) +
geom_point(aes(x=a, y=cdf)) 
...
```{r}
pnorm(2) #pnorm of 2 gives you 97% chance of observing 
probofGT2 <- 1-pnorm(2) # whats the probability of obtaining a value greater than two. It is 1 minus the probability of obtaining a value of 2 or less - this gives you 97%. So to obtain a value of greater than 2 it is 1 minus this value
```

- What is the probability of observing a value between -2 and 2?

```{r}
probof2to2 <- pnorm(2)-pnorm(-2) # taking the probability of obtaining a value of 2 or less (pnorm(2)), and removing the probability of obtaiing a value od -2 or less (this would be pnorm of -2)
probof2to2
```
...
```{r}
prob_extreme <- 1-(pnorm(2)-pnorm(-2))
prob_extreme
```
...
ecdf_iris_sepal_length(5) # tells you...?
```
...
```{r}
sort(unique(iris$Sepal.Length)) # knots generates the same output as sort - gives unique values in increasing order
```
...
```{r}
?pairwise.t.test # gives help page for pairwise t test function
```

```{r}
help(pairwise.t.test) # alternative help page
```
...
set.seed(1)
n_sample <- 10
x <- runif(n = n_sample, min = 10, max = 20) # runif used for random number generation in uniform distribution
...
# read in the data in sheet one and sheet 2 called logcounts_matrix and cell_info_matrix
logcounts <- read_excel("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx", sheet =1)
cell_info <- read_excel("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx", sheet = 2)
...
# tidy the data - convert it to longform
library(tidyverse)
log_counts_pivot <- pivot_longer(logcounts, cols=!gene, names_to= "Sample", values_to='counts')
log_counts_pivot

# perform join to merge the two sheets
log_counts_merge <- left_join(log_counts_pivot, cell_info, by="Sample") #do a left join to keeo all the rows
tail(log_counts_merge)
...
```{r}
# index so you move the name column to the side and can make a matrix out of integers
#column_to_rownames(logcounts, "gene")

logcounts_matrix <- as.matrix(column_to_rownames(logcounts, "gene"))
dim(logcounts_matrix)
head(logcounts_matrix)

cell_mock<- cell_info$Sample[cell_info$Infection=="Mock"]
head(cell_mock)

cell_infected <- cell_info$Sample[cell_info$Infection!="Mock"]
head(cell_infected)

gene1_mock<- logcounts_matrix[1,cell_mock]
gene1_infected <-logcounts_matrix[1,cell_infected]
head(gene1_mock)

# perform t-test
output <- t.test(gene1_mock, gene1_infected); output
...
t_test_row <- function(index) {
  general_gene_mock <- logcounts_matrix[index, cell_mock] 
  general_gene_infected <- logcounts_matrix[index, cell_infected] 
  output <- t.test(general_gene_mock, general_gene_infected)
  (output$p.value)}
